    ...
    public class TempCandidateLocalsController : Controller
    {
        ...
        public ActionResult Create_gh(TempCandidateLocalsViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    if (activeCandidates < model.MaxCandidates && candidate.national_id != null)
                    {
                        candidate.electionarea = model.ElectionArea;
                        candidate.city = model.City;
                        candidate.listname = model.ListName;

                        db.TempCandidateLocals.Add(candidate);
                        activeCandidates++;
                    }
                    else if (activeCandidates >= model.MaxCandidates)
                    {
                        break; // Stop adding if max candidates reached
                    }
                }
        ...
        public JsonResult GetUserData(int nationalNumber)
        {
            ...
            var partyCandidate = db.PartyCandidates.FirstOrDefault(p => p.national_id == nationalNumber);
            ...
            if (user != null)
            {
                ...
                {
                    return Json(new { error = "تم قبول هذا المترشح في أحد القوائم الحزبية" }, JsonRequestBehavior.AllowGet);
                }
                else if (age < 25)
                {
            ...
            return Json(new { error = "الرقم الوطني غير موجود" }, JsonRequestBehavior.AllowGet);
        }
