...
namespace JOVOICE.Controllers
{
    public class HomeController : Controller
    {
        ...
        public readonly ElectionEntities2 db = new ElectionEntities2();

        public ActionResult Index()
        {
            Session["partyPercent"] = Math.Round(((double)partyVotersCount() / (double)usersNumber()), 2) * 100;
            ...
            var startDate = db.ElictionsDates.Find(1).startDate;
            var endDate = db.ElictionsDates.Find(1).endDate;
            var currentDate = DateTime.Now;

            DateTime parsedStartDate = DateTime.Parse(startDate);
            DateTime parsedEndDate = DateTime.Parse(endDate);
            ViewBag.start = parsedStartDate;
            ViewBag.end = parsedEndDate;
        ...
        public ActionResult Contact(Feedback model)
        {
            if (ModelState.IsValid)
            {
                ...
                SendConfirmationEmail(model.email); // Assume model.Email contains the user's email
        ...
        private void SendConfirmationEmail(string toEmail)
        {
            ...
            string subjectText = "Feedback Received";
            string messageText = "Thank you for your feedback! We have received it and will get back to you soon.";
        ...
        public ActionResult Login(User user)
        {
            if (ModelState.IsValid)
            {
                ...
                if (existingUser != null)
                {
                    ...
                    {
                        // During the election period (between start date and end date)
        ...
        public ActionResult ListType(string id)
        {
        ...
        {
            var list = db.LocalLists.Where(w => w.electionDistrict.Trim() == id.Trim()).ToList();
